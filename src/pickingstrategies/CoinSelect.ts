...
export class CoinSelect extends PickingStrategy {
  ...
  async selectUnspentUtxosToUse(xpub: Xpub, amount: BigNumber, feePerByte: number, nbOutputsWithoutChange: number) {
    ...
    const oneInputSize = utils.estimateTxSize(1, 0, this.crypto, this.derivationMode) - fixedSize;
    ...
    const notInputFees = feePerByte * (fixedSize + oneOutputSize * nbOutputsWithoutChange);
    ...
    for (let i = 0; i < TOTAL_TRIES; i += 1) {
      ...
      if (currentValue >= actualTarget) {
        if (currentValue - actualTarget > feePerByte * oneOutputSize) {
          ...
          currentWaste =
            feePerByte * utils.estimateTxSize(nbInput, nbOutputsWithoutChange + 1, this.crypto, this.derivationMode);
        } else {
          // changeoutput is not required
          currentSelectionNeedChangeoutput = false;
          currentWaste =
            feePerByte * utils.estimateTxSize(nbInput, nbOutputsWithoutChange, this.crypto, this.derivationMode) +
            currentValue -
            actualTarget;
        }
    ...
    if (bestSelection.length > 0) {
      ...
      const fee =
        ...
        utils.estimateTxSize(
          ...
          bestSelectionNeedChangeoutput ? nbOutputsWithoutChange + 1 : nbOutputsWithoutChange,
          this.crypto,
          this.derivationMode
        );
      return {
