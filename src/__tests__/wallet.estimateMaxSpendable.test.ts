...
describe('testing estimateMaxSpendable', () => {
  ...
  it('should estimate max spendable correctly', async () => {
    ...
    let maxSpendable = await wallet.estimateAccountMaxSpendable(account, 0, [], true);
    ...
    let feesPerByte = 100;
    maxSpendable = await wallet.estimateAccountMaxSpendable(account, feesPerByte, [], true);
